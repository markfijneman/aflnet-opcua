...
region_t* extract_requests_test(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref)
{
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
  ...
  while (byte_count < buf_size) {
    ...
    //Check if the last two bytes are 0x0D0A
    if ((mem_count > 1) && (memcmp(&mem[mem_count], terminator, 1) == 0)) {
      region_count++;
      regions = (region_t *)ck_realloc(regions, region_count * sizeof(region_t));
      regions[region_count - 1].start_byte = cur_start;
      regions[region_count - 1].end_byte = cur_end;
...
unsigned int* extract_response_codes_test(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref)
{
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
...
// HERE
  while (byte_count < buf_size) {
    memcpy(&mem[mem_count], buf + byte_count++, 1);
    if ((mem_count > 0) && (memcmp(&mem[mem_count], terminator, 1) == 0)) {
      //Extract the response code which is the first 3 bytes
      char temp[4];
      memcpy(temp, mem, 4);
      temp[3] = 0x0;
      unsigned int message_code = (unsigned int) atoi(temp);
      // printf("Inside the if, code: %d\n",message_code);
      if (message_code == 0) break;
